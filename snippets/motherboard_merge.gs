...
function archiwizujSKAN() {
  ...
  if (mapa.length > 0) {
    if (totMapa.getLastRow() === 0) {
      totMapa.appendRow(["TOT", "Paleta", "STAGE", "EUT", "Data archiwizacji"]);
    }
    totMapa.getRange(totMapa.getLastRow() + 1, 1, mapa.length, 5).setValues(mapa);
  }

  sheet.clearContents();
}

function archiwizujRecznie() {
  archiwizujSKAN();
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Skanowanie");
  if (sheet) sheet.getRange("B2").setValue("✅ Skanuj EUT zielona komorka");
}
